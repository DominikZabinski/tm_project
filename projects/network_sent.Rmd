...
#' Convert CreateTextnet to igraph format
#'
#' @param text_network_graph CreateTextnet object
#' @param edge_df data.frame with additional information about edges
#'
#' @return
#' @export
#'
...
convert_to_igraph <- function(text_network_graph, edge_df) {
  # convert to igraph
  ll <- graph(edges = as_edgelist(text_network_graph))
  # assign weights
...
#' Creates a plot to show a graph
#'
#' @param graph_to_plot igraph object
#' @param scale_vertex_size numeric vector to scale vertex size to
#' @param edge_weight_limit numeric minimum value that edge weight should have for edge to be visible
#' @param idx_show_label number indices of vertices to show a label
#' @param scale_edge_size numeric vector to scale edge size to
#'
#' @return
#' @export
#'
...
match_colors <- function(x, tt) {
  ...
  avail_colors <- c(brewer.pal(8, "Dark2"), brewer.pal(8, "Set3"))
  nn <- unique(tt$ugr)
...
prepped_tagged <- lapply(X = period_bands$period, 
                         FUN = function(x){
                           PrepText(textdata = to_analyze_net[[x]],
                                    groupvar = "AUTHOR",
                                    textvar = "CONTENT",
                                    node_type = "groups",
                                    tokenizer = "words",
...
```{r, net_3}
pi_networks <- lapply(X = period_bands$period, FUN = function(x) CreateTextnet(tidytextobject = prepped_tagged[[x]]))
...
It is hard to see any specific patterns in any of those periods but there are clearly some subgroups forming in each period. Mean degree in each period was close to 3 but there were some members that *connected* to more than 150 members (table below). 
...
```{r, net_4}
pi_communities <- lapply(X = period_bands$period, FUN = function(x) TextCommunities(pi_networks[[x]]))
