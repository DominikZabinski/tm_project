...
plot_word_cloud <- function(cluster_words, specific_cluster) {
  wordcloud::wordcloud(words = names(cluster_words[[specific_cluster]]), 
...
Initial clustering gave an idea regarding most common topics in each term. Another way to tackle this problem is to use topic modelling. 

To start, for each term, I'll create list of top 6 topic using **Gibbs** method.
...
I'll try to show it differently
...
new_periods <- do.call(rbind,
                       lapply(X = period_bands$period,
                              FUN = function(x){
                                ...
                                ta <- seq(from = tt$start_date, by = 180, 
                                          length.out = ceiling((tt$end_date - tt$start_date) / 180))
...
# Evaluation (quality of obtained results)
